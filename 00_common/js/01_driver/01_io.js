...
My_entry.io.prototype.set_selection_elem = function(elem, je, sep){
  ...
  var len = 0;
  var script = self.entry.def.replace_commentAndWspace(self.read_text(elem));
  var sc = script.split(sep);
  for(var j=0, js=0, len_j=sc.length; j<len_j; ++j){
    var scj = sc[j];
    len += scj.length+len_sep;
    var mcj = scj.match(re);
    var hasOnlyRn = mcj && (mcj.length === scj.length);
    if(scj && !(hasOnlyRn) && ++js > je){
      break;
    }
  }
  self.set_selection(elem, len-len_sep);
  return self;
};
