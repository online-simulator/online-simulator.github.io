...
My_entry.handler_wave.prototype.str2freq = function(str, A4, tune, root, mode){  // Ver.1.84.15
  var self = this;
  var _freq = self.calc_freq(str, A4, tune, root, mode);  // Ver.1.44.11  // Ver.1.84.15
  /* Ver.1.65.14 -> */
  if(isNaN(_freq)){
...
My_entry.handler_wave.prototype.str2arr_f = function(str, A4, tune, root, mode){  // Ver.1.84.15
  var self = this;
  var _arr_f = [0];
  ...
  if(mcl){
    ...
    arr_f.forEach(function(str, k){
      arr_f[k] = self.str2freq(str, A4, tune, root, mode);  // Ver.1.84.15
    });
    f_isFound = arr_f;
  }
  else{
    f_isFound = [self.str2freq(str, A4, tune, root, mode)];  // Ver.1.84.15
  }
  if(f_isFound){
    _arr_f = f_isFound;
  }
  ...
  mcb.forEach(function(str, i){
    ...
    arr_data.forEach(function(tokens, j){
      ...
      if(hasDataset_base){
        ...
        params.arr_f = self.str2arr_f(arr_token[1], params.A4, params.tune, params.root, params.mode);  // Ver.1.73.14  // Ver.1.84.15
        /* -> Ver.1.20.4 */
